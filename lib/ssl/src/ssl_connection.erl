...
-export([send/2, recv/3, connect/7, ssl_accept/6, handshake/2,
	 ...
	 prf/5, negotiated_next_protocol/1]).
...
do_server_hello(Type, NextProtocolsToSend, #state{negotiated_version = Version,
    ...
    State = server_hello(ServerHello, State0#state{expecting_next_protocol_negotiation = NextProtocolsToSend =/= undefined}),
...
next_protocol(#state{expecting_next_protocol_negotiation = false} = State) ->
    State;
next_protocol(#state{transport_cb = Transport, socket = Socket, negotiated_version = Version, next_protocol = {_, NextProtocol}, connection_states = ConnectionStates0, tls_handshake_history = Handshake0} = State) ->
